...
bool Core::activate_with_context(ThreadContext& context, RAM& ram, mutex& output_mutex, float& cont_cach_hit) {
    ...
    while (relative_PC < context.end_address && quantum_remaing > 0) {
        try {
            uc.executarInstrucao(regs, ram, relative_PC, context.end_address,"data/setRegisters.txt", disco, Clock, instructions_executed, quantum_remaing, cache,cont_cach_hit);
...
bool Core::is_busy() const {
    ...
    return busy; // Supondo que 'busy' seja um membro booleano da classe Core
}